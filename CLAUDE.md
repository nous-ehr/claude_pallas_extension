...
You have access to 6 MCP tools that query the Athena Tools knowledge base (828 DataView views, 16K+ columns, 1.3K FK relationships, 1.9K API/FHIR/workflow docs). Use them. Never guess schema details, join columns, or API endpoints — always verify with the tools first.
...
Working reference implementations are in the `examples/` directory:
